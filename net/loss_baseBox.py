...
class yolov3_loss(nn.Module):
    ...
    def decode(self, x, y, w, h, bs, l_size, l, stride):
        '''
    ...
    def forward(self, predictions, targets):
        ...
        for l, prediction_s in enumerate(predictions):
            gt_trues = targets[l][targets[l][..., 4] == 1] #.view(-1, 5+self.num_classes)
            num_gt = gt_trues.size(0)
            target_mask = targets[l][..., 4] == 1
            l_size = l_sizes[l]
            # print(l_size)
            ...
            # print(prediction.shape)
            prediction = prediction_s.view(bs, 3, 5+self.num_classes, l_size, l_size).permute(0, 1, 3, 4, 2).contiguous() #[3,l,l,6]
            ...
            box_pred = self.decode(predx, predy, predw, predh, bs, l_size, l, self.input_shape/l_size)
            # print(box_pred.size())
            # print(conf.size())
            # print(cls.size())
            ...
            preds_boxes[..., :4] = preds_boxes[..., :4]/self.input_shape
            gt_trues[..., :4] = gt_trues[..., :4] / self.input_shape
            loss_x = self.MSELoss(preds_boxes[target_mask][..., 0], gt_trues[..., 0])
            loss_y = self.MSELoss(preds_boxes[target_mask][..., 1], gt_trues[..., 1])
            loss_w = self.MSELoss(preds_boxes[target_mask][..., 2], gt_trues[..., 2])
            loss_h = self.MSELoss(preds_boxes[target_mask][..., 3], gt_trues[..., 3])
            ...
            loss_cls_s = self.BCEloss(preds_boxes[target_mask][..., 5:], tar_cls)

            noobj_mask = torch.ones_like(preds_boxes[..., 4])
            noobj_mask[target_mask] = 0
            for b in range(bs):
                preds_box_tmp = torch.unsqueeze(preds_boxes[b], dim=-2).expand(3, l_size, l_size, gt_trues.size(0),
                                                                            5 + self.num_classes)
                iou1 = self.cal_iou(preds_box_tmp, gt_trues)
                maxiou1 = torch.max(iou1, dim=-1).values
                noobj_mask[b][maxiou1 >= self.ignore] = 0
            ...
            loss_conf_s = torch.sum(self.BCEloss(preds_boxes[target_mask][..., 4], 1)) + \
                          torch.sum(self.BCEloss(preds_boxes[noobj_mask==1][..., 4], 0))

            loss_reg += torch.sum(loss_reg_s)
            loss_conf += loss_conf_s
        ...
        reg_weight, conf_weight = 5, 0.5
        loss = (reg_weight*loss_reg + conf_weight*loss_conf + loss_cls) / num_gts
